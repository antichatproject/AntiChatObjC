...
+ (NSBitmapImageRep *)bitmapRepWithCVMat:(const cv::Mat&)cvMat
{
...
{
  self = [self initWithSize:NSMakeSize(cvMat.cols, cvMat.rows)];
  if (self) {
    NSBitmapImageRep *bitmapRep = nil;
    if (cvMat.elemSize() == 1) {
      bitmapRep = [[self class] bitmapRepWithCVMat:cvMat];
    } else {
      cv::Mat rgbMat;
      cv::cvtColor(cvMat, rgbMat, cv::COLOR_BGR2RGB);
      bitmapRep = [[self class] bitmapRepWithCVMat:rgbMat];
    }
